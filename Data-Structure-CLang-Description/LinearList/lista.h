...
static SqList *InitList(void);   // 构造一个空的线性表L
inline static SqList *CreatList(SqList *L); // 初始化一个长度为LIST_INIT_SIZE的线性表
inline static void DestroyList(SqList **LL);    // 销毁线性表L
inline static void CleanList(SqList *L);    // 将L重置为空表
inline static int ListEmpty(SqList *L); // 若L为空表，则返回TRUE,否则返回FALSE
inline static int ListLength(SqList *L); // 返回L中的数据元素的长度
inline static ElemType GetElem(SqList *L, int i);    // 用e返回L中第i个数据元素的值
inline static int LocateElem(SqList *L, ElemType e);   // 返回L中第1个与e满足关系compare()的数据元素
inline static int Compare(ElemType elem, ElemType e); // 如果满足compare()返回TRUE,否则返回FALSE
inline static int ListChange(SqList *L, ElemType e, int i); // 将第i个元素的值修改为e
// inline static int PriorElem();   // 若cur_e是L的数据元素，且不是第一个，则用pre_e返回它的前驱，否则操作失败，pre_e无定义
// inline static int NextElem();    // 若cur_e是L的数据元素，且不是最后一个，则用next_e返回它的后继，否则操作失败，nest_e无定义
inline static int ListInsert(SqList **LL, int i, ElemType e);   // 在第i个位置之前插入元素e
inline static int ListDelete(SqList *L, int i); // 删除L的第i个数据元素，并用e返回其值
// inline static int ListTraverse();
inline static void ShowList(const SqList *L);   // ShowList
